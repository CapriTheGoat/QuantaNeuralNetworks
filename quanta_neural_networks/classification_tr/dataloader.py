...
class TRMNISTAsynchronousDataset(Dataset):
    def __init__(
        self,
        h5_file_path: str | Path,
    ):
        """
        Reads continuous passive TR-SPAD data from an HDF5 file 
        and returns the event stream.
        
        :param h5_file_path: Path to the TR-MNIST .h5 dataset file.
        :param max_events: Optional cutoff to prevent memory spikes on extremely 
                           bright samples (truncates to first N photons).
        """
        self.h5_path = Path(h5_file_path)
        
        # Pre-index the dataset to map indices to digit/index strings
        self.samples = []
        with h5py.File(self.h5_path, 'r') as f:
            for digit in f.keys():
                for index_str in f[digit].keys():
                    self.samples.append((digit, index_str))
    ...
    def __getitem__(self, index):
        try:
            digit, index_str = self.samples[index]
            target_label = torch.tensor(int(digit), dtype=torch.long)
            
            # Read the 1D arrays from the HDF5 file
            with h5py.File(self.h5_path, 'r') as f:
                x = f[digit][index_str]["x"][:]
                y = f[digit][index_str]["y"][:]
                t = f[digit][index_str]["t"][:]
                
            # Combine into a single event array of shape [N, 3] -> (y, x, t)
            events = np.stack([y, x, t], axis=1)
