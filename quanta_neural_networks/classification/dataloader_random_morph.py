...
def stochastic_spad_morph(video_a, video_b, label_A, label_B):
    """
    Morphs two SPAD videos together by swapping pixels probabilistically.
    Both videos must be the same shape: [Batch, Height, Width, Time]
    """
    ...
    start_wipe = random.randint(20,80)/100 - (wipe_fraction / 2)

    target_label = torch.full((video_a.shape[0], T), -100, dtype=torch.long, device=device)
    morph_point = int(T * start_wipe)
    target_label[:, :morph_point] = label_A.unsqueeze(1)
    target_label[:, morph_point:] = label_B.unsqueeze(1)
    ...
    return torch.where(use_video_b, video_b, video_a), target_label
