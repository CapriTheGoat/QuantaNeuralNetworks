...
class PerPixelBayesian(nn.Module):
    ...
    @torch.compile
    def _get_reconstruction_torch(self, delta_t_cube: Tensor, photon_mask: Tensor, time_per_frame: float, t_absolute: int):
        """
        ...
        for t_index in range(self._t):
            ...
            run_time = estimated_run_length * time_per_frame
            decay_factor = torch.exp(-delta_t / run_time.clamp(min=1e-6))
            
            self.ema = (self.ema * decay_factor) + mask
            self._update_forecaster_and_laplace_torch(delta_t, mask, t_index + t_absolute)

            if (t_index + 1) % self.subsampling == 0:
                self.recons_tensor[..., t_index // self.subsampling] = self.ema

    def _update_forecaster_and_laplace_torch(self, delta_t: Tensor, mask: Tensor, t_index: int):
        ...
        log_likelihood = (
            ...
            + self.alpha_ll * torch.log(self.beta_ll / (self.beta_ll + delta_t))
        )
        ...
        self.forecaster_distribution = torch.where(
            mask_ == 1.0, 
            self.forecaster_distribution * (1 - self.bocpd_gamma) * likelihood, 
            self.forecaster_distribution
        )
        ...
        mask_should_insert = (mask_has_capacity | mask_should_replace) & (mask_.squeeze(-1) == 1.0)
        ...
        self.forecaster_distribution.scatter_(
            ...
            torch.where(
                ...
                new_forecaster_prob.unsqueeze(-1), 
                self.forecaster_distribution.gather(-1, insert_indices_expanded)
            )
        )
        ...
        self.alpha_ll.scatter_(
            -1, 
            insert_indices_expanded, 
            torch.where(
                mask_insert_expanded, 
                1.0, 
                self.alpha_ll.gather(-1, insert_indices_expanded)
            )
        )
        
        self.beta_ll.scatter_(
            -1, 
            insert_indices_expanded, 
            torch.where(
                mask_insert_expanded, 
                1.0, 
                self.beta_ll.gather(-1, insert_indices_expanded)
            )
        )
